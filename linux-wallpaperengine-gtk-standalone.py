...
# Auto-generated from src/ structure
...
"""
SettingsDialog module for Linux Wallpaper Engine GTK
Extracted from mature monolith v0.2.1
"""
...
# From wallpaper_context_menu.py
"""
WallpaperContextMenu module for Linux Wallpaper Engine GTK
Extracted from mature monolith v0.2.1
"""
...
# From wallpaper_engine.py
"""
WallpaperEngine module for Linux Wallpaper Engine GTK
Extracted from mature monolith v0.2.1
"""
...
class WallpaperEngine:
    ...
    def _detect_display(self):
        ...
        try:
            ...
            result = subprocess.run(  # nosec
                "xrandr | grep -w 'primary'", shell=True, capture_output=True, text=True
            )
            if result.stdout:
            ...
            result = subprocess.run(  # nosec
                "xrandr | grep -w 'connected'", shell=True, capture_output=True, text=True
            )
            if result.stdout:
                display = result.stdout.split()[0]
    ...
    def run_wallpaper(self, wallpaper_id, **options):
        ...
        try:
            ...
            self.log.debug(
                f"Single-process detection: enable_custom_args="
                f"{options.get('enable_custom_args')}, custom_args='{options.get('custom_args')}', "
                f"using_single_process={using_single_process}"
            )
            ...
            if not using_single_process:
                ...
                self.log.info(
                    "Single-process mode detected - excluding problematic "
                    "scaling/clamping arguments"
                )
                if options.get("fps"):
                    cmd.extend(["--fps", str(options["fps"])])
                if options.get("mute"):
    ...
    def stop_wallpaper(self):
        ...
        try:
            ...
            while True:
                result = subprocess.run(  # nosec
                    ["pgrep", "-f", f"{self.wpe_path}\\b"], capture_output=True, text=True
                )
...
# From wallpaper_window.py
"""
WallpaperWindow module for Linux Wallpaper Engine GTK
Extracted from mature monolith v0.2.1
"""
