...
class ᴜɪ:
    ...
    def ᴅ(title: str, parent: Optional[Gtk.Window]=None, msg: str='') -> None:
        ...
        ᴅ = Gtk.MessageDialog(transient_for=parent, flags=0, message_type=Gtk.MessageType.ERROR, buttons=Gtk.ButtonsType.OK, text=title)
        if msg:
...
class ᴡᴇ:
    ...
    def __init__(ꜱ, wpe_path=None):
        ...
        ꜱ.ᴡ = None
        ꜱ.ᴄ = None
        ꜱ.ᴡᴘ = wpe_path or ꜱ.ᴘ.ꜰ('wpe', ['~/linux-wallpaperengine/build/linux-wallpaperengine', '~/src/linux-wallpaperengine/build/linux-wallpaperengine', '/usr/local/bin/linux-wallpaperengine', '/usr/bin/linux-wallpaperengine', os.path.expanduser('~/bin/linux-wallpaperengine')])
        ꜱ.ᴡᴅ = ꜱ.ᴘ.ꜰ('wallpapers', ['~/.steam/steam/steamapps/workshop/content/431960', '~/.steam/debian-installation/steamapps/workshop/content/431960', '~/.local/share/Steam/steamapps/workshop/content/431960'])

    def _ᴅᴅ(ꜱ) -> Optional[str]:
        """Detect display"""
        try:
            ʀ = subprocess.run("xrandr | grep -w 'primary'", shell=True, capture_output=True, text=True)
            if ʀ.stdout:
                ᴅ = ʀ.stdout.split()[0]
                ꜱ.ʟ.ɪɴꜰ(f'Found primary: {ᴅ}')
                return ᴅ
            ʀ = subprocess.run("xrandr | grep -w 'connected'", shell=True, capture_output=True, text=True)
            if ʀ.stdout:
                ᴅ = ʀ.stdout.split()[0]
                ꜱ.ʟ.ɪɴꜰ(f'Using: {ᴅ}')
                return ᴅ
    ...
    def ʀ(ꜱ, wid: str, **opts) -> Tuple[bool, Optional[List[str]]]:
        ...
        try:
            ...
            p = subprocess.Popen(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, stdin=subprocess.DEVNULL, start_new_session=True, close_fds=True, cwd=wd)
            time.sleep(0.1)
            if p.poll() is None:
                ꜱ.ᴡ = wid
                ꜱ.ᴄ = p
    ...
    def stop(ꜱ) -> bool:
        ...
        try:
            ...
            while True:
                r = subprocess.run(['pgrep', '-f', f'{ꜱ.ᴡᴘ}\\b'], capture_output=True, text=True)
                ...
                time.sleep(0.1)
                c = subprocess.run(['pgrep', '-f', f'{ꜱ.ᴡᴘ}\\b'], capture_output=True)
                ...
                if c.returncode == 0:
                    ...
                    time.sleep(0.1)
                f = subprocess.run(['pgrep', '-f', f'{ꜱ.ᴡᴘ}\\b'], capture_output=True)
...
class G(Gtk.Window):
    ...
    def __init__(s, wpe_path=None):
        ...
        s.s = S({'fps': 60, 'volume': 100, 'mute': False, 'mouse_enabled': True, 'auto_rotation': False, 'rotation_interval': 15, 'no_automute': False, 'no_audio_processing': False, 'no_fullscreen_pause': False, 'scaling': 'default', 'clamping': 'clamp'})
        s.pw = 200
        s.ph = 120
        s.pt = None
        s.pa = False
        ...
        s.lw()
        if not s._v():
            U.d('System State Invalid', s, 'Could not find required components.\n' + 'Please ensure linux-wallpaperengine is installed.')
    ...
    def _ui(s):
        ...
        s.fb.connect('child-activated', s.ow)
        s.fb.connect('button-press-event', s.or_)
        sc.add(s.fb)
    ...
    def _tb(s):
        ...
        s.vs.connect('value-changed', s.ov)
        ...
        sc.set_value(100)
        sc.connect('value-changed', s.op)
        pb.pack_start(sc, True, True, 0)
        ps.add(pb)
        tb.insert(ps, -1)
        for icon, tip, cb in [('media-skip-backward-symbolic', 'Previous', s.bp), ('media-skip-forward-symbolic', 'Next', s.bn), ('media-playlist-shuffle-symbolic', 'Random', s.br), ('preferences-system-symbolic', 'Settings', s.bs)]:
            b = Gtk.ToolButton()
    ...
    def _hg(s, path: str, box: Gtk.Box) -> None:
        ...
        if a.is_static_image():
            p = a.get_static_image().scale_simple(s.pw, s.ph, GdkPixbuf.InterpType.BILINEAR)
        ...
        while True:
            ...
            sp = p.scale_simple(nw, nh, GdkPixbuf.InterpType.BILINEAR)
            fs.append(sp)
            if not it.advance():
                break
        i = Gtk.Image()
    ...
    def _hs(s, path: str, box: Gtk.Box) -> None:
        ...
        p = GdkPixbuf.Pixbuf.new_from_file_at_scale(path, s.pw, s.ph, True)
        i = Gtk.Image.new_from_pixbuf(p)
        box.add(i)
...
class D(Gtk.Dialog):
    ...
    def __init__(s, parent: G, settings: dict):
        ...
        s._pt()
        s._at()
        s.add_buttons(Gtk.STOCK_CANCEL, Gtk.ResponseType.CANCEL, Gtk.STOCK_SAVE, Gtk.ResponseType.OK)
    ...
    def _dt(s) -> None:
        ...
        cols = [('Enabled', 0, True), ('Directory', 1, True), ('Total', 2, False), ('Blacklisted', 3, False), ('Active', 4, False), ('Last Scan', 5, False)]
        for t, i, e in cols:
            if i == 0:
                r = Gtk.CellRendererToggle()
                r.connect('toggled', s.dt)
        ...
        bb.set_spacing(6)
        for l, cb in [('Add Directory', s.ad), ('Remove Directory', s.rd), ('Scan All', s.sa)]:
    ...
    def _at(s) -> None:
        ...
        sws = [('Keep Playing When Other Apps Play Sound', 'no_automute'), ('Disable Audio Effects', 'no_audio_processing')]
    ...
    def _pd(s) -> None:
        ...
        for p, i in s.p.dm.directories.items():
            ...
            s.ds.append([i['enabled'], p, st['total_count'], st['blacklisted_count'], st['active_count'], st['last_scan'].split('T')[0] if st.get('last_scan') else 'Never'])
    ...
    def bw(s, button: Gtk.Button) -> None:
        """Browse WPE"""
        d = Gtk.FileChooserDialog(title='Select WPE Executable', parent=s, action=Gtk.FileChooserAction.OPEN)
    ...
    def ad(s, button: Gtk.Button) -> None:
        """Add directory"""
        d = Gtk.FileChooserDialog(title='Select Directory', parent=s, action=Gtk.FileChooserAction.SELECT_FOLDER)
        d.add_buttons(Gtk.STOCK_CANCEL, Gtk.ResponseType.CANCEL, Gtk.STOCK_OPEN, Gtk.ResponseType.OK)
        if d.run() == Gtk.ResponseType.OK:
    ...
    def rd(s, button: Gtk.Button) -> None:
        ...
        if it:
            p = m[it][1]
            c = Gtk.MessageDialog(transient_for=s, message_type=Gtk.MessageType.QUESTION, buttons=Gtk.ButtonsType.YES_NO, text=f'Remove directory {p}?')
    ...
    def on_response(s, dialog: Gtk.Dialog, response: int) -> None:
        ...
        if response == Gtk.ResponseType.OK:
            s.p.s.c.update({'fps': s.fs.get_value_as_int(), 'volume': int(s.vs.get_value()), 'no_automute': s.no_automute_switch.get_active(), 'no_audio_processing': s.no_audio_processing_switch.get_active(), 'process_priority': s.pc.get_active_text()})
            wp = s.we.get_text()
            if wp != s.p.e.wp:
                s.p.e.wp = wp
            s.p.s.save()